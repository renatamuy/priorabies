...
# hospital distance (values represent travel time to hospitals)
skewed_values <- rexp(100, rate = 0.1) 

hospital_distance <- rast(nrows = 10, ncols = 10, vals = skewed_values)
crs(hospital_distance) <- "EPSG:4326"  # Assign CRS (WGS84)

# Create toy raster for road density (random values between 0 and 1)
road_density <- rast(nrows = 10, ncols = 10, vals = runif(100, 0, 1))
crs(road_density) <- "EPSG:4326"  # Assign CRS

# Create toy raster for population density (random values between 0 and 1000)
population_density <- rast(nrows = 10, ncols = 10, vals = sample(1:1000, 100, replace = TRUE))
crs(population_density) <- "EPSG:4326"  # Assign CRS
...
crs(dog_density) <- "EPSG:4326"  # Assign CRS (WGS84)

# vampire bat presence (binary values: 0 = no bats, 1 = bats present)
bat_presence <- rast(nrows = 10, ncols = 10, vals = sample(0:1, 100, replace = TRUE))
crs(bat_presence) <- "EPSG:4326"  # Assign CRS
...
# Combine into a single benefit index (weighted sum) just to think about them!
benefit <- (0.5 * dog_density) + (0.3 * bat_presence) + (0.2 * rabies_cases)

# Normalize the benefit index / risk raster
...
crs(city_raster) <- "EPSG:4326"  # Assign CRS (WGS84)
...
cities_sf <- st_sf(city_id = 1:5, geometry = city_polygons)

# Check
plot(cities_sf["city_id"], main = "Manually Defined Non-overlapping City Polygons")
# Print the sf object
brazil_pu <- cities_sf
